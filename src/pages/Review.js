...
const ReviewPage = () => {
  ...
  const handlePlaceChanged = async () => {
    ...
    try {
      const response = await fetch('https://travelcheck-hzdwesazbcead2bm.canadaeast-01.azurewebsites.net/locations', {
        method: 'POST',
      ...
      const reviewsResponse = await fetch(`https://travelcheck-hzdwesazbcead2bm.canadaeast-01.azurewebsites.net/reviews?location_id=${locationData.id}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('https://travelcheck-hzdwesazbcead2bm.canadaeast-01.azurewebsites.net/reviews', {
        method: 'POST',
  ...
  const handleLike = async (reviewId) => {
    ...
    try {
      const response = await fetch(`https://travelcheck-hzdwesazbcead2bm.canadaeast-01.azurewebsites.net/reviews/${reviewId}/like`, {
        method: 'POST',
        headers: {
  ...
  const handleCommentSubmit = async (e, reviewId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://travelcheck-hzdwesazbcead2bm.canadaeast-01.azurewebsites.net/reviews/${reviewId}/comments`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const fetchCommentsForReview = async (reviewId, page = 1) => {
    try {
      const response = await fetch(
        `https://travelcheck-hzdwesazbcead2bm.canadaeast-01.azurewebsites.net/reviews/${reviewId}/comments?page=${page}&limit=${COMMENTS_PER_PAGE}`
      );
      const data = await response.json();
