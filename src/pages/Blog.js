...
const BlogPage = () => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const commentsResponse = await fetch(`http://localhost:5000/blogs/${blogId}/comments`);
  ...
  const loadMoreComments = async (blogId, page) => {
    try {
      const response = await fetch(
        `http://localhost:5000/blogs/${blogId}/comments?page=${page}&limit=${COMMENTS_PER_PAGE}`
      );
