...
const BlogPage = () => {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        const response = await fetch('https://travelcheck.azurewebsites.net/blogs');
        if (response.ok) {
  ...
  const handleLike = async (blogId, liked) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://travelcheck.azurewebsites.net/blogs/${blogId}/like`, {
        method: 'POST',
        headers: {
  ...
  try {
    const token = localStorage.getItem('token');
    const response = await fetch(`https://travelcheck.azurewebsites.net/blogs/${blogId}/comments`, {
      method: 'POST',
      headers: {
    ...
    if (response.ok) {
      ...
      const commentsResponse = await fetch(`https://travelcheck.azurewebsites.net/blogs/${blogId}/comments`);
  ...
  const loadMoreComments = async (blogId, page) => {
    try {
      const response = await fetch(
        `https://travelcheck.azurewebsites.net/blogs/${blogId}/comments?page=${page}&limit=${COMMENTS_PER_PAGE}`
      );
