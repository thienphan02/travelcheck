...
const SettingsPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserInfo = async () => {
      try {
        const response = await fetch('https://travelcheck.azurewebsites.net/users/me', { // Adjust the endpoint to fetch the current user
          headers: {
  ...
  const handleUpdateUserInfo = async () => {
    const token = localStorage.getItem('token');
    try {
      const response = await fetch('https://travelcheck.azurewebsites.net/users/me', {
        method: 'PUT',
        headers: {
