...
const ManageUsers = () => {
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      try {
        const response = await fetch('https://travelcheck-hzdwesazbcead2bm.canadaeast-01.azurewebsites.net/users', {
          headers: {
  ...
  const handleDelete = async (id) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const response = await fetch(`https://travelcheck-hzdwesazbcead2bm.canadaeast-01.azurewebsites.net/users/${id}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const handleSaveEdit = async () => {
    ...
    const token = localStorage.getItem('token');
    try {
      const response = await fetch(`https://travelcheck-hzdwesazbcead2bm.canadaeast-01.azurewebsites.net/users/${editUser.id}`, {
        method: 'PUT',
        headers: {
